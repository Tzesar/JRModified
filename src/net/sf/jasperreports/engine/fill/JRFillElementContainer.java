...
import static net.sf.jasperreports.engine.fill.ElumenRichTextHelper.processRichText;
...
public abstract class JRFillElementContainer extends JRFillElementGroup implements FillContainerContext
{
	...
	{
        System.out.println("-----BEGIN SetDependentElements-----");
        if (ySortedElements != null && ySortedElements.length > 0)
		{
			for(int i = 0; i < ySortedElements.length - 1; i++)
			{
                ...
                // My Changes - Augusto
                try{
                    System.out.println("Element: " + ((JRFillTextField) iElem).getText());
				...
				for(int j = i + 1; j < ySortedElements.length; j++)
				{
						...
						)
					{
                        // My Changes - Augusto
                        try {
                            if ( ((JRFillTextField) jElem).getStyledText() != null )
                                System.out.println("New dependant element: " + ((JRFillTextField) jElem).getText() + " y=" + jElem.getY());
                        } catch (ClassCastException ex){}
                        // End of changes
        ...
        System.out.println("-----END SetDependantElements-----");
    }
