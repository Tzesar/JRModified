...
public abstract class JRFillElementContainer extends JRFillElementGroup implements FillContainerContext
{
    ...
    private void setDependentElements()
    {
        ...
        {
            for (JRFillElement fillElement : ySortedElements){
                try {
                    System.out.println("ySortedPrintText: " + ((JRFillTextField) fillElement).getText()+" - y: " + ((JRFillTextField) fillElement).getY() + " - height: " + ((JRFillTextField) fillElement).getHeight());
    ...
    protected void evaluate(byte evaluation) throws JRException
    {
        ...
        {
            System.out.println("All elements length: "+allElements.length);
            for(int i = 0; i < allElements.length; i++)
            {
                ...
                try{
                    JRFillTextField fillTextField = (JRFillTextField)element;
                    System.out.println("Text before process: "+fillTextField.getText());
                    processRichText(element);
                } catch (ClassCastException ex){
                    System.out.println("Error: "+ex.getMessage());
                }
            }
            initElements();
//            Collections.sort(Arrays.asList(this.elements), new JRYComparator());
//            ySortedElements = new JRFillElement[elements.length];
//            Arrays.asList(this.elements).toArray(ySortedElements);
//            setDependentElements();
        }
        //}
    }
    ...
    // my changes
    public List<JRFillElement> processRichText(JRElement element) throws JRException {
        ...
        if(element instanceof JRTextField){
            ...
            if ( textField.getText() != null ){
                String elementContent = textField.getText().replaceAll("&nbsp;", "");
                System.out.println("Text: " + elementContent);

                if (elementContent != null && !Jsoup.isValid(elementContent, Whitelist.none())) {
                    System.out.println("IS RICH TEXT");
...
//                        elementList.add(imageElement);
                    ...
                    this.elements = new JRFillElement[elementList.size()];
                    elementList.toArray(this.elements);
                    for (JRFillElement fillElement : this.elements){
                        try {
                            System.out.println("PrintText: " + ((JRFillTextField) fillElement).getText());
                        } catch (ClassCastException ex){

                        }
                    }
                }else{
                    System.out.println("NO RICH TEXT");
    ...
    public static Whitelist getStyledWhitelist() {
    ...
    {
        System.out.println("-----STRETCH ELEMENTS-----");
        if (stretchElements != null && stretchElements.length > 0)
        {
            for(int i = 0; i < stretchElements.length; i++)
            {
                ...
                try {
                    System.out.println("Stretch Elements Text: " + ((JRFillTextField) element).getText());
                } catch (ClassCastException ex){

                }
                element.stretchElement(stretchHeight - getContainerHeight());//TODO subtract firstY?
                try {
                    System.out.println("Move dependant elements of Text: " + ((JRFillTextField) element).getText());
                } catch (ClassCastException ex){
        ...
        if (ySortedElements != null && ySortedElements.length > 0)
        {
            for(int i = 0; i < ySortedElements.length; i++)
            {
                JRFillElement element = ySortedElements[i];
                try {
                    System.out.println("Stretch FINAL Text: " + ((JRFillTextField) element).getText());
                } catch (ClassCastException ex){
    ...
    protected void moveBandBottomElements()
    {
        //if (!willOverflow)
        //{
        System.out.println("-----MOVE BAND BOTTOM ELEMENTS-----");
        if (bandBottomElements != null && bandBottomElements.length > 0)
        {
            for(int i = 0; i < bandBottomElements.length; i++)
            {
                ...
                try {
                    System.out.println("Element Text: " + ((JRFillTextField) element).getText());
                } catch (ClassCastException ex){
    ...
    {
        System.out.println("-----REMOVE BLANK ELEMENTS-----");
        ...
        if (remElems != null && remElems.length > 0)
        {
            ...
            for(int i = 0; i < remElems.length; i++)
            {
                ...
                try {
                    System.out.println("Maybe remove Text: " + ((JRFillTextField) iElem).getText());
                } catch (ClassCastException ex){
                        ...
                        )
                {
                    ...
                    {
                        try {
                            System.out.println("For remove Text: " + ((JRFillTextField) iElem).getText());
                        } catch (ClassCastException ex){
